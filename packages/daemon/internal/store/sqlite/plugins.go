...
// an empty `tenantID` argument to NULL transparently.
package sqlite
...
// nullableTenantArg returns sql.NullString for the WHERE clause.
func nullableTenantArg(tenantID string) any {
	if tenantID == "" {
		return nil
	}
	return tenantID
...
// scopeWhere builds a "tenant_scope = ? OR (tenant_scope IS NULL AND ?)"
// fragment that matches either a specific tenant or a global row.
// For simplicity we use a single equality and rely on the caller to
// pick global vs tenant via the second arg.
...
func (t *tx) CreatePlugin(ctx context.Context, in *store.Plugin) (*store.Plugin, error) {
	if in == nil || in.Slug == "" || in.Name == "" || in.Version == "" {
		return nil, fmt.Errorf("sqlite: plugin requires slug, name, version")
	}
	id := in.ID
	if id == "" {
		id = newID("plug")
	...
	if err != nil {
		...
		return nil, fmt.Errorf("sqlite: insert plugin: %w", err)
	}
...
func (t *tx) UpdatePlugin(ctx context.Context, tenantID, id string, p store.UpdatePluginParams) (*store.Plugin, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx,
		...
		return nil, fmt.Errorf("sqlite: update plugin: %w", err)
	}
...
func (t *tx) CreatePluginSigner(ctx context.Context, in *store.PluginSigner) (*store.PluginSigner, error) {
	if in == nil || in.Name == "" || in.Fingerprint == "" {
		return nil, fmt.Errorf("sqlite: plugin_signer requires name, fingerprint")
	}
	id := in.ID
	if id == "" {
		id = newID("psign")
	...
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE") && strings.Contains(err.Error(), "fingerprint") {
			return nil, store.ErrPluginSignerFPTaken
		}
		return nil, fmt.Errorf("sqlite: insert plugin_signer: %w", err)
	}
...
func (t *tx) ListPluginSignersByScope(ctx context.Context, tenantID string) ([]*store.PluginSigner, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: list signers: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (t *tx) UpdatePluginSigner(ctx context.Context, tenantID, id string, p store.UpdatePluginSignerParams) (*store.PluginSigner, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx,
		...
		return nil, fmt.Errorf("sqlite: update signer: %w", err)
	}
...
func (t *tx) DeletePluginSigner(ctx context.Context, tenantID, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: delete signer: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (t *tx) ListPluginsBySigner(ctx context.Context, signerID string) ([]*store.Plugin, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: list plugins by signer: %w", err)
	}
	defer func() { _ = rows.Close() }()
	var out []*store.Plugin
	for rows.Next() {
...
// Suppress unused-warning for nullableTenantArg if added later.
var _ = nullableTenantArg

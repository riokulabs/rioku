...
func (t *tx) CreateDashboard(ctx context.Context, in *store.Dashboard) (*store.Dashboard, error) {
	if in == nil {
		return nil, fmt.Errorf("sqlite: nil dashboard")
	}
	if in.TenantID == "" || in.Name == "" {
		return nil, fmt.Errorf("sqlite: dashboard requires tenant_id, name")
	}
	id := in.ID
	if id == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: insert dashboard: %w", err)
	}
...
func (t *tx) ListDashboardsByTenant(ctx context.Context, tenantID string) ([]*store.Dashboard, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: list dashboards: %w", err)
	}
...
func (t *tx) UpdateDashboard(ctx context.Context, tenantID, id string, params store.UpdateDashboardParams) (*store.Dashboard, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: update dashboard: %w", err)
	}
...
func (t *tx) DeleteDashboard(ctx context.Context, tenantID, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: delete dashboard: %w", err)
	}
...
func (t *tx) SetDashboardHomeForUser(ctx context.Context, tenantID, id, userID string) (*store.Dashboard, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: scan dashboards: %w", err)
	}
	type pair struct {
		id   string
		json string
	...
	for _, p := range dashboards {
		...
		if _, err := t.sqlTx.ExecContext(ctx,
			...
			return nil, fmt.Errorf("sqlite: update home_for_users: %w", err)
		}
	}
	t.emit("dashboards", id, "UPDATE")
	return t.GetDashboard(ctx, tenantID, id)
...
func (t *tx) CreateWidget(ctx context.Context, in *store.Widget) (*store.Widget, error) {
	if in == nil {
		return nil, fmt.Errorf("sqlite: nil widget")
	}
	if in.DashboardID == "" || in.Kind == "" || in.Title == "" {
		return nil, fmt.Errorf("sqlite: widget requires dashboard_id, kind, title")
	}
	id := in.ID
	if id == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: insert widget: %w", err)
	}
...
func (t *tx) ListWidgetsByDashboard(ctx context.Context, dashboardID string) ([]*store.Widget, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: list widgets: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (t *tx) UpdateWidget(ctx context.Context, id string, params store.UpdateWidgetParams) (*store.Widget, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: update widget: %w", err)
	}
...
func (t *tx) DeleteWidget(ctx context.Context, dashboardID, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: delete widget: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
...
// pair in `layouts` within a single transaction context. Widget IDs
// not present in the dashboard are ignored — the route handler
// already verified ownership before calling.
func (t *tx) UpdateDashboardLayout(ctx context.Context, dashboardID string, layouts map[string]string) error {
	...
	for widgetID, layout := range layouts {
		if _, err := t.sqlTx.ExecContext(ctx,
			...
			return fmt.Errorf("sqlite: update layout for %s: %w", widgetID, err)
		}
	}
	return nil
}
...
func (t *tx) CreateDashboardVersion(ctx context.Context, in *store.DashboardVersion) (*store.DashboardVersion, error) {
	if in == nil {
		return nil, fmt.Errorf("sqlite: nil version")
	}
	if in.DashboardID == "" || in.SnapshotJSON == "" {
		return nil, fmt.Errorf("sqlite: version requires dashboard_id, snapshot_json")
	}
	id := in.ID
	if id == "" {
	...
	if err := t.sqlTx.QueryRowContext(ctx,
		...
		return nil, fmt.Errorf("sqlite: max version: %w", err)
	}
	next := int32(1)
	if maxVer.Valid {
		next = maxVer.Int32 + 1
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: insert version: %w", err)
	}

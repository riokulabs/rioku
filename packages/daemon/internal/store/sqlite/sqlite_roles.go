...
func (t *tx) GetRole(ctx context.Context, id string) (*store.Role, error) {
	...
	row := t.sqlTx.QueryRowContext(ctx,
		`SELECT id, name, description, is_builtin, parent_role_id, created_at, updated_at FROM roles WHERE id = ? AND (tenant_id IS NULL OR tenant_id = ?)`, id, tenantID)
	r := &store.Role{}
	var isBuiltinInt int
	var parentRoleID sql.NullString
	var createdStr, updatedStr string
	if err := row.Scan(&r.ID, &r.Name, &r.Description, &isBuiltinInt, &parentRoleID, &createdStr, &updatedStr); err != nil {
...
func (t *tx) ListRoles(ctx context.Context) ([]*store.Role, error) {
	tenantID := store.TenantIDFromContext(ctx)
	rows, err := t.sqlTx.QueryContext(ctx,
		`SELECT id, name, description, is_builtin, parent_role_id, created_at, updated_at FROM roles WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY name`, tenantID)
	if err != nil {
	...
	for rows.Next() {
		...
		var parentRoleID sql.NullString
		var createdStr, updatedStr string
		if err := rows.Scan(&r.ID, &r.Name, &r.Description, &isBuiltinInt, &parentRoleID, &createdStr, &updatedStr); err != nil {
			return nil, err
		}
		r.IsBuiltin = isBuiltinInt == 1
		if parentRoleID.Valid {
...
func (t *tx) UpdateRole(ctx context.Context, id string, params store.UpdateRoleParams) (*store.Role, error) {
	...
	if params.ClearParent {
		if _, err := t.sqlTx.ExecContext(ctx,
			...
			return nil, fmt.Errorf("sqlite: clear parent_role_id: %w", err)
		}
		...
		if _, err := t.sqlTx.ExecContext(ctx,
			...
			*params.ParentRoleID, now, id, tenantID,
			...
			return nil, fmt.Errorf("sqlite: set parent_role_id: %w", err)
		}
	}
...
// the catalog. Re-registering a permission that's already owned by
// the same plugin is a no-op (idempotent). Re-registering a built-in
// or different-plugin id returns ErrPermissionConflict.
func (t *tx) RegisterPluginPermissions(ctx context.Context, pluginID string, perms []*store.Permission) error {
	if pluginID == "" {
		return fmt.Errorf("sqlite: register plugin permissions: pluginID required")
	}
	for _, p := range perms {
		if p == nil || p.ID == "" {
			continue
		...
		err := t.sqlTx.QueryRowContext(ctx,
			`SELECT source, COALESCE(source_plugin_id, '') FROM permissions WHERE id = ?`, p.ID,
		...
		if err == nil {
			...
			// Idempotent — refresh description in case the manifest changed.
			if _, err := t.sqlTx.ExecContext(ctx,
				`UPDATE permissions SET description = ? WHERE id = ?`,
				p.Description, p.ID); err != nil {
				return fmt.Errorf("sqlite: update plugin permission: %w", err)
			}
			continue
		}
		if _, err := t.sqlTx.ExecContext(ctx,
			...
			return fmt.Errorf("sqlite: insert plugin permission %q: %w", p.ID, err)
		}
	}
	return nil
}

// UnregisterPluginPermissions deletes catalog rows whose source is
// "plugin-manifest" and source_plugin_id equals the supplied id.
// Built-in rows are untouched.
func (t *tx) UnregisterPluginPermissions(ctx context.Context, pluginID string) (int, error) {
	if pluginID == "" {
		return 0, fmt.Errorf("sqlite: unregister plugin permissions: pluginID required")
	}
	...
	if err != nil {
		return 0, fmt.Errorf("sqlite: unregister plugin permissions: %w", err)
	}

...
func (t *tx) CreateSsoProvider(ctx context.Context, in *store.SsoProvider) (*store.SsoProvider, error) {
	if in == nil || in.TenantID == "" || in.Name == "" || in.Kind == "" {
		return nil, fmt.Errorf("sqlite: sso_provider requires tenant_id, name, kind")
	}
	id := in.ID
	if id == "" {
		id = newID("sso")
	...
	_, err := t.sqlTx.ExecContext(ctx,
		...
		nullStr(in.OIDCIssuer), nullStr(in.OIDCClientID), nullStr(in.OIDCClientSecretRef),
	...
	if err != nil {
		...
		return nil, fmt.Errorf("sqlite: insert sso_provider: %w", err)
	}
...
func (t *tx) ListSsoProvidersByTenant(ctx context.Context, tenantID string) ([]*store.SsoProvider, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: list sso_providers: %w", err)
	}
...
func (t *tx) UpdateSsoProvider(ctx context.Context, tenantID, id string, p store.UpdateSsoProviderParams) (*store.SsoProvider, error) {
	...
	if p.OIDCIssuer != nil {
		cur.OIDCIssuer = strPtrOrNil(*p.OIDCIssuer)
	}
	if p.OIDCClientID != nil {
		cur.OIDCClientID = strPtrOrNil(*p.OIDCClientID)
	}
	if p.OIDCClientSecretRef != nil {
		cur.OIDCClientSecretRef = strPtrOrNil(*p.OIDCClientSecretRef)
	}
	if p.OIDCScopes != nil {
	...
	if _, err := t.sqlTx.ExecContext(ctx,
		...
		cur.Name, cur.Kind,
		nullStr(cur.OIDCIssuer), nullStr(cur.OIDCClientID), nullStr(cur.OIDCClientSecretRef),
		cur.OIDCScopes, cur.ClaimsMapping, boolToInt(cur.Enabled), nowUTC(), id, tenantID); err != nil {
		if isUniqueNameErr(err) {
			return nil, store.ErrSsoProviderTaken
		}
		return nil, fmt.Errorf("sqlite: update sso_provider: %w", err)
	}
...
func (t *tx) DeleteSsoProvider(ctx context.Context, tenantID, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: delete sso_provider: %w", err)
	}
...
func scanSsoProvider(s scanner) (*store.SsoProvider, error) {
	...
	return &store.SsoProvider{
		...
		OIDCIssuer:          nullStrPtr(issuer),
		OIDCClientID:        nullStrPtr(clientID),
		OIDCClientSecretRef: nullStrPtr(secretRef),
		OIDCScopes:          scopes,
...
// nullStr returns sql.NullString from *string (nil/"" -> NULL).
...
// strPtrOrNil normalises empty to nil for *string column writes.
...
// nullStrPtr maps sql.NullString back to *string.
...
// isUniqueNameErr is the sqlite-specific UNIQUE constraint detector.
// We check the message rather than the error code to stay portable across
// the modernc and mattn driver flavours.
func isUniqueNameErr(err error) bool {
	if err == nil {
		return false
	}
	msg := err.Error()
	return strings.Contains(msg, "UNIQUE") && strings.Contains(msg, "name")
}

...
func (t *tx) CreatePasswordResetToken(ctx context.Context, tokenHash, userID string, expiresAt time.Time) error {
	...
	_, err := t.sqlTx.ExecContext(ctx,
		`INSERT INTO password_reset_tokens (token, user_id, created_at, expires_at)
		 VALUES (?, ?, ?, ?)`,
		tokenHash, userID, now, expiresAt.UTC().Format(timeFormat),
	)
	if err != nil {
		return fmt.Errorf("sqlite: create password reset token: %w", err)
	}
	return nil
}
...
func (t *tx) GetPasswordResetToken(ctx context.Context, tokenHash string) (*store.PasswordResetToken, error) {
	...
	if err := row.Scan(&tok.TokenHash, &tok.UserID, &createdAt, &expiresAt, &consumedAt); err != nil {
		return nil, fmt.Errorf("sqlite: get password reset token: %w", err)
	}
	var err error
	...
	if consumedAt.Valid {
		...
		if err != nil {
			return nil, fmt.Errorf("sqlite: parse consumed_at: %w", err)
		}
		tok.ConsumedAt = &t2
	}
	return &tok, nil
...
func (t *tx) ConsumePasswordResetToken(ctx context.Context, tokenHash string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: consume password reset token: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("sqlite: token not found or already consumed")
	}
	return nil
}

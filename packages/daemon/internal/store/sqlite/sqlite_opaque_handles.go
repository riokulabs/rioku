...
func (t *tx) GetOpaqueHandle(ctx context.Context, tenantID, handle string) (*store.OpaqueHandle, error) {
	...
	h, err := scanOpaqueHandle(row)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite: get opaque handle: %w", err)
	}
	return h, nil
}
...
func (t *tx) GetOpaqueHandleByValueHash(ctx context.Context, tenantID, valueHash string) (*store.OpaqueHandle, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: get opaque handle by value hash: %w", err)
	}
	return h, nil
}
...
func (t *tx) UpsertOpaqueHandle(ctx context.Context, h store.OpaqueHandle) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: upsert opaque handle: %w", err)
	}
	return nil
}

// scanOpaqueHandle scans a single row into an OpaqueHandle.
func scanOpaqueHandle(row interface {
	Scan(dest ...any) error

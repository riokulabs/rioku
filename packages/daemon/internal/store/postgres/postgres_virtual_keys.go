...
func (t *tx) CreateVirtualKey(ctx context.Context, p store.CreateVirtualKeyParams) (*store.VirtualKey, error) {
	...
	allowedJSON, err := marshalAllowedModelsPg(p.AllowedModels)
	if err != nil {
		return nil, err
	}
	if p.BudgetWindow == "" {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("postgres: create virtual_key: %w", err)
	}
...
func (t *tx) GetVirtualKey(ctx context.Context, id string) (*store.VirtualKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: get virtual_key: %w", err)
	}
	return k, nil
}
...
func (t *tx) ListVirtualKeys(ctx context.Context) ([]*store.VirtualKey, error) {
	...
	rows, err := t.sqlTx.QueryContext(ctx, rewritePlaceholders(
		...
		tenantID)
	if err != nil {
		return nil, fmt.Errorf("postgres: list virtual_keys: %w", err)
	}
...
func (t *tx) UpdateVirtualKey(ctx context.Context, id string, p store.UpdateVirtualKeyParams) (*store.VirtualKey, error) {
	...
	if p.AllowedModels != nil {
		j, err := marshalAllowedModelsPg(*p.AllowedModels)
		if err != nil {
			return nil, err
		}
		sets = append(sets, "allowed_models = ?::jsonb")
		args = append(args, j)
	}
	if p.RPMLimit != nil {
	...
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(q), args...)
	if err != nil {
		if isPgUniqueViolation(err, "") {
			return nil, store.ErrVirtualKeyNameTaken
		}
		return nil, fmt.Errorf("postgres: update virtual_key: %w", err)
	}
...
func (t *tx) RotateVirtualKey(ctx context.Context, id, newCredentialRef string) (*store.VirtualKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: rotate virtual_key: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return nil, store.ErrVirtualKeyNotFound
	}
...
func (t *tx) RevokeVirtualKey(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres: revoke virtual_key: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (t *tx) DeleteVirtualKey(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres: delete virtual_key: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return store.ErrVirtualKeyNotFound
	}
...
// marshalAllowedModelsPg returns a JSON array string suitable for the
// virtual_keys.allowed_models JSONB column. nil becomes an empty array.
func marshalAllowedModelsPg(models []string) (string, error) {
	if models == nil {
		models = []string{}
	}

...
func (t *tx) CreateAIProvider(ctx context.Context, in *store.AIProvider) (*store.AIProvider, error) {
	if in == nil || in.TenantID == "" || in.Name == "" || in.Kind == "" {
		return nil, fmt.Errorf("postgres: ai_provider requires tenant_id, name, kind")
	}
	id := in.ID
	if id == "" {
		id = newID("aiprov")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		in.Enabled, meta, now, now,
	)
	if err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrAIProviderNameTaken
		}
		return nil, fmt.Errorf("postgres: insert ai_provider: %w", err)
	}
...
func (t *tx) ListAIProvidersByTenant(ctx context.Context, tenantID string) ([]*store.AIProvider, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list ai_providers: %w", err)
	}
...
func (t *tx) UpdateAIProvider(ctx context.Context, tenantID, id string, p store.UpdateAIProviderParams) (*store.AIProvider, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		c.Name, c.Kind, c.BaseURL, c.Credential, c.Enabled, c.Metadata, now, id, tenantID); err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrAIProviderNameTaken
		}
		return nil, fmt.Errorf("postgres: update ai_provider: %w", err)
	}
...
func (t *tx) DeleteAIProvider(ctx context.Context, tenantID, id string) error {
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`DELETE FROM ai_providers WHERE id = ? AND tenant_id = ?`), id, tenantID)
	if err != nil {
		return fmt.Errorf("postgres: delete ai_provider: %w", err)
	}
...
func scanAIProvider(s scanner) (*store.AIProvider, error) {
	var (
		id, tenantID, name, kind, baseURL, metadata string
		credential                                  *string
		enabled                                     bool
	...
	return &store.AIProvider{
		...
		Credential: credential, Enabled: enabled, Metadata: metadata,
...
func (t *tx) AddProviderModel(ctx context.Context, in *store.AIProviderModel) (*store.AIProviderModel, error) {
	if in == nil || in.ProviderID == "" || in.UpstreamModelID == "" || in.Alias == "" {
		return nil, fmt.Errorf("postgres: provider_model requires provider_id, upstream_model_id, alias")
	}
	id := in.ID
	if id == "" {
		id = newID("aimodel")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`INSERT INTO ai_provider_models (id, provider_id, upstream_model_id, alias, rate_limit_rpm, daily_quota_tokens, enabled, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`),
		id, in.ProviderID, in.UpstreamModelID, in.Alias, in.RateLimitRPM, in.DailyQuotaTokens, in.Enabled, now, now,
	)
	if err != nil {
		return nil, fmt.Errorf("postgres: insert provider_model: %w", err)
	}
...
func (t *tx) UpdateProviderModel(ctx context.Context, providerID, modelID string, p store.UpdateAIProviderModelParams) (*store.AIProviderModel, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		c.Alias, c.RateLimitRPM, c.DailyQuotaTokens, c.Enabled, nowUTC(), modelID, providerID); err != nil {
		return nil, fmt.Errorf("postgres: update provider_model: %w", err)
	}
...
func (t *tx) RemoveProviderModel(ctx context.Context, providerID, modelID string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres: delete provider_model: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (t *tx) ListProviderModels(ctx context.Context, providerID string) ([]*store.AIProviderModel, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list provider_models: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func scanProviderModel(s scanner) (*store.AIProviderModel, error) {
	var (
		id, providerID, upstreamID, alias string
		rateLimitRPM                      int32
		dailyQuota                        int64
		enabled                           bool
		createdAt, updatedAt              time.Time
	...
	return &store.AIProviderModel{
		...
		RateLimitRPM: rateLimitRPM, DailyQuotaTokens: dailyQuota, Enabled: enabled,
		CreatedAt: createdAt.UTC(), UpdatedAt: updatedAt.UTC(),
	}, nil
}
...
func (t *tx) CreateMCPServer(ctx context.Context, in *store.AIMCPServer) (*store.AIMCPServer, error) {
	if in == nil || in.TenantID == "" || in.Name == "" || in.URL == "" {
		return nil, fmt.Errorf("postgres: mcp_server requires tenant_id, name, url")
	}
	id := in.ID
	if id == "" {
		id = newID("mcp")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`INSERT INTO ai_mcp_servers (id, tenant_id, name, url, auth_kind, auth_credential, health, enabled,
		 ...
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),
		id, in.TenantID, in.Name, in.URL, authKind, in.AuthCredential, health, in.Enabled,
		authorized, lastCheckedAt, now, now,
	)
	if err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrMCPServerNameTaken
		}
		return nil, fmt.Errorf("postgres: insert mcp_server: %w", err)
	}
...
func (t *tx) ListMCPServersByTenant(ctx context.Context, tenantID string) ([]*store.AIMCPServer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list mcp_servers: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (t *tx) UpdateMCPServer(ctx context.Context, tenantID, id string, p store.UpdateAIMCPServerParams) (*store.AIMCPServer, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`UPDATE ai_mcp_servers SET name=?, url=?, auth_kind=?, auth_credential=?, health=?, enabled=?,
		 ...
		 WHERE id=? AND tenant_id=?`),
		c.Name, c.URL, c.AuthKind, c.AuthCredential, c.Health, c.Enabled,
		c.AuthorizedAgentIDs, lastCheckedAt, nowUTC(), id, tenantID); err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrMCPServerNameTaken
		}
		return nil, fmt.Errorf("postgres: update mcp_server: %w", err)
	}
...
func (t *tx) DeleteMCPServer(ctx context.Context, tenantID, id string) error {
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`DELETE FROM ai_mcp_servers WHERE id = ? AND tenant_id = ?`), id, tenantID)
	if err != nil {
		return fmt.Errorf("postgres: delete mcp_server: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func scanMCPServer(s scanner) (*store.AIMCPServer, error) {
	var (
		id, tenantID, name, url, authKind, health, authorized string
	...
	out := &store.AIMCPServer{
		...
		AuthCredential: authCredential, Health: health, Enabled: enabled,
		AuthorizedAgentIDs: authorized,
		CreatedAt:          createdAt.UTC(), UpdatedAt: updatedAt.UTC(),
	}
	if lastCheckedAt.Valid {
		ts := lastCheckedAt.Time.UTC()
...
func (t *tx) CreateAITool(ctx context.Context, in *store.AITool) (*store.AITool, error) {
	if in == nil || in.TenantID == "" || in.Name == "" || in.Kind == "" {
		return nil, fmt.Errorf("postgres: ai_tool requires tenant_id, name, kind")
	}
	id := in.ID
	if id == "" {
		id = newID("aitool")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		in.Dangerous, in.Enabled, now, now,
	)
	if err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrAIToolNameTaken
		}
		return nil, fmt.Errorf("postgres: insert ai_tool: %w", err)
	}
...
func (t *tx) ListAIToolsByTenant(ctx context.Context, tenantID string) ([]*store.AITool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list ai_tools: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (t *tx) UpdateAITool(ctx context.Context, tenantID, id string, p store.UpdateAIToolParams) (*store.AITool, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		c.Dangerous, c.Enabled, nowUTC(), id, tenantID); err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrAIToolNameTaken
		}
		return nil, fmt.Errorf("postgres: update ai_tool: %w", err)
	}
...
func (t *tx) DeleteAITool(ctx context.Context, tenantID, id string) error {
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`DELETE FROM ai_tools WHERE id = ? AND tenant_id = ?`), id, tenantID)
	if err != nil {
		return fmt.Errorf("postgres: delete ai_tool: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func scanAITool(s scanner) (*store.AITool, error) {
	var (
		id, tenantID, name, kind, description, schemaJSON string
		httpEndpoint, mcpServerID                         *string
		dangerous, enabled                                bool
		createdAt, updatedAt                              time.Time
	)
	...
	return &store.AITool{
		...
		Dangerous: dangerous, Enabled: enabled,
		CreatedAt: createdAt.UTC(), UpdatedAt: updatedAt.UTC(),
	}, nil
}
...
func (t *tx) CreateAIAgent(ctx context.Context, in *store.AIAgent) (*store.AIAgent, error) {
	if in == nil || in.TenantID == "" || in.Name == "" {
		return nil, fmt.Errorf("postgres: ai_agent requires tenant_id, name")
	}
	id := in.ID
	if id == "" {
		id = newID("aiagent")
	...
	if err != nil {
		...
		return nil, fmt.Errorf("postgres: insert ai_agent: %w", err)
	}
...
func (t *tx) UpdateAIAgent(ctx context.Context, tenantID, id string, p store.UpdateAIAgentParams) (*store.AIAgent, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		return nil, fmt.Errorf("postgres: update ai_agent: %w", err)
	}
...
func (t *tx) DeleteAIAgent(ctx context.Context, tenantID, id string) error {
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`DELETE FROM ai_agents WHERE id = ? AND tenant_id = ?`), id, tenantID)
	if err != nil {
		return fmt.Errorf("postgres: delete ai_agent: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (t *tx) CreateAIToolBinding(ctx context.Context, in *store.AIToolBinding) (*store.AIToolBinding, error) {
	if in == nil || in.TenantID == "" || in.AgentID == "" || in.ToolID == "" {
		return nil, fmt.Errorf("postgres: tool_binding requires tenant_id, agent_id, tool_id")
	}
	id := in.ID
	if id == "" {
		id = newID("aibind")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`INSERT INTO ai_tool_bindings (id, tenant_id, agent_id, tool_id, condition, enabled, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`),
		id, in.TenantID, in.AgentID, in.ToolID, in.Condition, in.Enabled, now, now,
	)
	if err != nil {
		if isPgUniqueViolation(err, "") {
			return nil, store.ErrAIBindingExists
		}
		return nil, fmt.Errorf("postgres: insert tool_binding: %w", err)
	}
...
func (t *tx) ListAIToolBindingsByTenant(ctx context.Context, tenantID string) ([]*store.AIToolBinding, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list tool_bindings: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (t *tx) ListAIToolBindingsByAgent(ctx context.Context, agentID string) ([]*store.AIToolBinding, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list tool_bindings_by_agent: %w", err)
	}
	defer func() { _ = rows.Close() }()
	var out []*store.AIToolBinding
	for rows.Next() {
...
func (t *tx) UpdateAIToolBinding(ctx context.Context, tenantID, id string, p store.UpdateAIToolBindingParams) (*store.AIToolBinding, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`UPDATE ai_tool_bindings SET condition=?, enabled=?, updated_at=?
		 WHERE id=? AND tenant_id=?`),
		c.Condition, c.Enabled, nowUTC(), id, tenantID); err != nil {
		return nil, fmt.Errorf("postgres: update tool_binding: %w", err)
	}
...
func (t *tx) DeleteAIToolBinding(ctx context.Context, tenantID, id string) error {
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`DELETE FROM ai_tool_bindings WHERE id = ? AND tenant_id = ?`), id, tenantID)
	if err != nil {
		return fmt.Errorf("postgres: delete tool_binding: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func scanAIToolBinding(s scanner) (*store.AIToolBinding, error) {
	var (
		id, tenantID, agentID, toolID, condition string
		enabled                                  bool
		createdAt, updatedAt                     time.Time
	)
	...
	return &store.AIToolBinding{
		...
		Enabled: enabled, CreatedAt: createdAt.UTC(), UpdatedAt: updatedAt.UTC(),
	}, nil
}
...
func (t *tx) CreateAIRateLimit(ctx context.Context, in *store.AISemanticRateLimit) (*store.AISemanticRateLimit, error) {
	if in == nil || in.TenantID == "" || in.Name == "" || in.Scope == "" {
		return nil, fmt.Errorf("postgres: rate_limit requires tenant_id, name, scope")
	}
	id := in.ID
	if id == "" {
		id = newID("airl")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		in.SimilarityThreshold, in.WindowSeconds, in.Threshold, action, in.Enabled, now, now,
	)
	if err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrAIRateLimitNameTaken
		}
		return nil, fmt.Errorf("postgres: insert rate_limit: %w", err)
	}
...
func (t *tx) ListAIRateLimitsByTenant(ctx context.Context, tenantID string) ([]*store.AISemanticRateLimit, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list rate_limits: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (t *tx) UpdateAIRateLimit(ctx context.Context, tenantID, id string, p store.UpdateAIRateLimitParams) (*store.AISemanticRateLimit, error) {
	...
	if _, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		c.SimilarityThreshold, c.WindowSeconds, c.Threshold, c.Action, c.Enabled, nowUTC(),
		id, tenantID); err != nil {
		if isPgUniqueViolation(err, "name") {
			return nil, store.ErrAIRateLimitNameTaken
		}
		return nil, fmt.Errorf("postgres: update rate_limit: %w", err)
	}
...
func (t *tx) DeleteAIRateLimit(ctx context.Context, tenantID, id string) error {
	res, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		`DELETE FROM ai_semantic_rate_limits WHERE id = ? AND tenant_id = ?`), id, tenantID)
	if err != nil {
		return fmt.Errorf("postgres: delete rate_limit: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func scanAIRateLimit(s scanner) (*store.AISemanticRateLimit, error) {
	var (
		id, tenantID, name, scope, exemplars, action string
	...
	return &store.AISemanticRateLimit{
		...
		Threshold: threshold, Action: action, Enabled: enabled,
		CreatedAt: createdAt.UTC(), UpdatedAt: updatedAt.UTC(),
	}, nil
}
...
func (t *tx) AppendAITrace(ctx context.Context, in *store.AITrace) (*store.AITrace, error) {
	if in == nil || in.TenantID == "" || in.Status == "" {
		return nil, fmt.Errorf("postgres: trace requires tenant_id, status")
	}
	id := in.ID
	if id == "" {
		id = newID("aitrace")
	...
	_, err := t.sqlTx.ExecContext(ctx, rewritePlaceholders(
		...
		in.OutputTokens, in.DurationMS, in.Prompt, in.Completion, calls, in.Error, occurredAt,
	)
	if err != nil {
...
func scanAITrace(s scanner) (*store.AITrace, error) {
	var (
		id, tenantID, model, status, calls string
		agentID, providerID                *string
		prompt, completion, errStr         *string

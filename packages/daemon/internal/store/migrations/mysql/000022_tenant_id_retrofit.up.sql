...
-- Services
ALTER TABLE services ADD COLUMN tenant_id VARCHAR(64) NOT NULL DEFAULT 'tenant_default' REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX idx_services_tenant ON services (tenant_id);

-- Policies
ALTER TABLE policies ADD COLUMN tenant_id VARCHAR(64) NOT NULL DEFAULT 'tenant_default' REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX idx_policies_tenant ON policies (tenant_id);

-- API Keys (already has owner_id)
ALTER TABLE api_keys ADD COLUMN tenant_id VARCHAR(64) NOT NULL DEFAULT 'tenant_default' REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX idx_api_keys_tenant ON api_keys (tenant_id);

-- Audit log
ALTER TABLE audit_log ADD COLUMN tenant_id VARCHAR(64) NOT NULL DEFAULT 'tenant_default' REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX idx_audit_log_tenant ON audit_log (tenant_id, occurred_at DESC);

-- Sessions
ALTER TABLE sessions ADD COLUMN tenant_id VARCHAR(64) NOT NULL DEFAULT 'tenant_default' REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX idx_sessions_tenant ON sessions (tenant_id);

-- Access policies (added in migration 8 without tenant_id)
ALTER TABLE access_policies ADD COLUMN tenant_id VARCHAR(64) NOT NULL DEFAULT 'tenant_default' REFERENCES tenants(id) ON DELETE CASCADE;
...
ALTER TABLE roles ADD COLUMN tenant_id VARCHAR(64) REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX idx_roles_tenant ON roles (tenant_id);

INSERT IGNORE INTO schema_versions (version, dirty) VALUES (22, 0);

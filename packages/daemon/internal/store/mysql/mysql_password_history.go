...
func (t *tx) fetchUserHash(ctx context.Context, userID string) (string, error) {
	...
	if err := row.Scan(&hash); err != nil {
		...
		return "", fmt.Errorf("mysql: fetch user hash: %w", err)
	}
	return hash, nil
}
...
func (t *tx) fetchPasswordHistoryDepth(ctx context.Context, tenantID string) (int, error) {
	...
	case sql.ErrNoRows:
		return 5, nil
	default:
		return 0, fmt.Errorf("mysql: fetch password history depth: %w", err)
	}
}
...
func (t *tx) checkHistoryReuse(ctx context.Context, userID, newHash string, depth int) error {
	...
	if err != nil {
		return fmt.Errorf("mysql: fetch password history: %w", err)
	}
...
func (t *tx) commitNewPassword(ctx context.Context, userID, prevHash, newHash string, depth int) error {
	...
	if _, err := t.sqlTx.ExecContext(ctx,
		...
		return fmt.Errorf("mysql: update password: %w", err)
	}
	...
	if prevHash != "" {
		if _, err := t.sqlTx.ExecContext(ctx,
			...
			return fmt.Errorf("mysql: insert password history: %w", err)
		}
	}
	...
	//
	// MySQL forbids referring to the target table in a DELETE
	// subquery, so we wrap the inner SELECT in a derived table.
	limit := depth - 1
	if limit < 0 {
		limit = 0
	}
	if _, err := t.sqlTx.ExecContext(ctx,
		...
		return fmt.Errorf("mysql: trim password history: %w", err)
	}

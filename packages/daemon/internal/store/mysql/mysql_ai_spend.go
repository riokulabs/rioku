...
func (t *tx) AppendAISpendLog(ctx context.Context, log *store.AISpendLog) error {
	if log == nil || log.ID == "" || log.ModelID == "" {
		return fmt.Errorf("mysql: ai_spend_log requires id + model_id")
	}
	...
	if err != nil {
		return fmt.Errorf("mysql: append ai_spend_log: %w", err)
	}
	return nil
}
...
func (t *tx) QueryAISpendLogs(ctx context.Context, q store.AISpendQuery) ([]*store.AISpendLog, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql: query ai_spend_logs: %w", err)
	}
...
func (t *tx) PruneAISpendLogs(ctx context.Context, before time.Time) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("mysql: prune ai_spend_logs: %w", err)
	}
...
// model, date) PK — re-runs upsert via ON DUPLICATE KEY UPDATE.
// Returns the row count processed for telemetry.
func (t *tx) AggregateAISpendDay(ctx context.Context, day time.Time) (int64, error) {
	...
	// MySQL stores created_at as a TIMESTAMP; SUBSTRING(... , 1, 10)
	// over the ISO-8601 string we wrote yields the UTC date prefix.
	rows, err := t.sqlTx.QueryContext(ctx,
		`SELECT COALESCE(virtual_key_id, ''), model_id,
		 ...
		 FROM ai_spend_logs
		 WHERE tenant_id = ?
		   AND SUBSTRING(created_at, 1, 10) = ?
		 GROUP BY virtual_key_id, model_id`,
		tenantID, dayStr,
	)
	if err != nil {
		return 0, fmt.Errorf("mysql: aggregate spend: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		_, err := t.sqlTx.ExecContext(ctx,
			`INSERT INTO ai_spend_rollups (tenant_id, virtual_key_id, model_id, rollup_date,
			 ...
			 ON DUPLICATE KEY UPDATE
			     ...
			     input_tokens_total = VALUES(input_tokens_total),
			     output_tokens_total = VALUES(output_tokens_total),
			     total_tokens = VALUES(total_tokens),
			     cost_usd_total = VALUES(cost_usd_total),
		...
		if err != nil {
			return processed, fmt.Errorf("mysql: upsert ai_spend_rollups: %w", err)
		}
		processed++
	}
...
func (t *tx) QueryAISpendRollups(ctx context.Context, q store.AISpendRollupQuery) ([]*store.AISpendRollup, error) {
	...
	rows, err := t.sqlTx.QueryContext(ctx, sqlStr, args...)
	if err != nil {
		return nil, fmt.Errorf("mysql: query ai_spend_rollups: %w", err)
	}
	defer func() { _ = rows.Close() }()

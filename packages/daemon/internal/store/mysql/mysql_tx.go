...
func (t *tx) CreateService(ctx context.Context, svc *riokuv1.Service) (*riokuv1.Service, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql: marshal request_headers: %w", err)
	}
	respHJSON, err := marshalResponseHeadersJSON(svc.GetResponseHeaders())
	if err != nil {
		return nil, fmt.Errorf("mysql: marshal response_headers: %w", err)
	}
	respRulesJSON, err := marshalResponseRulesJSON(svc.GetResponseRules())
	if err != nil {
		return nil, fmt.Errorf("mysql: marshal response_rules: %w", err)
	}
	compJSON, err := marshalCompressionJSON(svc.GetCompression())
	if err != nil {
		return nil, fmt.Errorf("mysql: marshal compression: %w", err)
	}
...
// Webhooks, Cluster Enrollment Tokens, and Impersonation Sessions are implemented in mysql_webhooks_cluster_impersonation.go.

// Settings config singletons are implemented in mysql_settings_configs.go.

// PKI/TLS subsystem methods are implemented in mysql_pki.go.

// Plugins and PluginSigners are implemented in mysql_plugins.go.

// Notifications subsystem methods are implemented in mysql_notifications.go.

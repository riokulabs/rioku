...
page load, not just on `/api-explorer` visits. However, the _execution_ of Scalar's
...
```ts
if (id.includes('/node_modules/zod/') || id.includes('/node_modules/.pnpm/zod@')) return 'zod';
```

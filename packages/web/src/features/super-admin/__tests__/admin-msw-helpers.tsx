 ...
 * them, so the literal values in raw items must come from the chain logger.
 *
 * To produce a real chain in tests: import `logAdminAuditEntry` from
 * `@/api/resources/audit`, call it N times, then read
 * `useMockStore.getState().adminAudit` and pass that array as the audit items
 * (the hash chain is real — the daemon's eventual implementation must yield
 * the same SHA-256 output for the same JSON payload).
 */

/**
 * Registry tests — asserts the 20 built-in types are present, each entry has
 ...
 * Rioku system, plus 5 more (bar-chart, donut, funnel, markdown, progress)
 * during stage-1 close-out.
 */
import { describe, expect, it } from 'vitest';
import { BUILT_IN_WIDGETS, BUILT_IN_WIDGET_IDS, getBuiltInWidget } from '../registry';

describe('BUILT_IN_WIDGETS registry', () => {
  it('registers all 20 built-in widget types', () => {
    const expected = [
      'single-stat',
      'sparkline',
      'time-series',
    ...
    expect(BUILT_IN_WIDGET_IDS).toHaveLength(20);
  });

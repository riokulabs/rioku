...
export const createSiteWizardSchema = z
  .object({
    ...
    rate_limit_preset: z.enum(['none', 'lenient', 'standard', 'strict']).default('none'),
    redirect_rules: z
      .array(
        z.object({
  ...
  .superRefine((v, ctx) => {
    ...
    if (v.upstream_mode === 'new_upstream' && (!v.upstream_host || !v.upstream_protocol)) {
      ctx.addIssue({
        code: 'custom',
        path: ['upstream_host'],
        message: 'Required',
    ...
    if (v.tls_mode === 'manual' && (!v.tls_manual_cert_pem || !v.tls_manual_key_pem)) {
      ctx.addIssue({
        code: 'custom',

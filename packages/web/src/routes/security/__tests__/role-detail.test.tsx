...
vi.mock('lucide-react', () => ({ ArrowLeftIcon: () => <span />, TrashIcon: () => <span />, SaveIcon: () => <span data-testid="save-icon" /> }))
...
describe('RoleDetailPage', () => {
  ...
  it('renders save button for non-builtin role', () => { render(<RoleDetailPage />); expect(screen.getByText('Save changes')).toBeInTheDocument() })
  it('save button is disabled when no changes', () => { render(<RoleDetailPage />); const saveBtn = screen.getByText('Save changes').closest('button')!; expect(saveBtn).toBeDisabled() })
})
